...
main:
	...
	li	a3,-2147356672
	li	a5,-2147352576
	lui	a0,%hi(Next_Ptr_Glob)
	lui	s7,%hi(Ptr_Glob)
	sw	ra,156(sp)
	sw	a3,%lo(Next_Ptr_Glob)(a0)
	sw	s0,152(sp)
	sw	a5,%lo(Ptr_Glob)(s7)
	sw	s1,148(sp)
	sw	s2,144(sp)
	sw	s3,140(sp)
	sw	s4,136(sp)
...
#NO_APP
	...
	addi	a0,s6,%lo(.LC19)
	call	printf
	lui	s6,%hi(Next_Ptr_Glob)
	lw	a5,%lo(Next_Ptr_Glob)(s6)
	lw	a0,0(a5)
	call	printhex
	addi	a0,s8,%lo(.LC20)
	call	printf
