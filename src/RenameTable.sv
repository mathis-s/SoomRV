...
(
    ...
    input dup_info IN_lookupIDs_info[NUM_LOOKUP-1:0],
    ...
    input dup_info IN_issueIDs_info[NUM_ISSUE-1:0],
...
logic[TAG_SIZE-1:0] comTag_dup[NUM_REGS-1:0] /*verilator public*/;
logic[TAG_SIZE-1:0] specTag_dup[NUM_REGS-1:0] /*verilator public*/;

reg[NUM_TAGS-1:0] tagAvail /*verilator public*/;

always_comb begin
    for (integer i = 0; i < NUM_LOOKUP; i=i+1) begin
        if(IN_lookupIDs_info[i].original) begin
            OUT_lookupSpecTag[i] = specTag[IN_lookupIDs[i]];
        end else begin
            OUT_lookupSpecTag[i] = specTag_dup[IN_lookupIDs[i]];
        end
        ...
        for (integer j = 0; j < (i / 2); j=j+1) begin
            if (IN_issueValid[j] && IN_issueIDs[j] == IN_lookupIDs[i] && IN_issueIDs[j] != 0) begin
                if(!IN_issueIDs_info[j].duplicated || (IN_issueIDs_info[j].original == IN_lookupIDs_info[i].original)) begin
    ...
    for (integer i = 0; i < NUM_COMMIT; i=i+1) begin
        ...
        OUT_commitPrevTags_dup[i] = comTag_dup[IN_commitIDs[i]];
    end
end
...
always_ff@(posedge clk /*or posedge rst*/) begin
    ...
    if (rst) begin
        ...
        for (integer i = 0; i < NUM_REGS; i=i+1) begin
            comTag[i] <= TAG_ZERO;
            specTag[i] <= TAG_ZERO;
            comTag_dup[i] <= TAG_ZERO;
            specTag_dup[i] <= TAG_ZERO;
        end
        ...
        if (IN_mispred) begin
            for (integer i = 1; i < NUM_REGS; i=i+1) begin
                ...
                specTag[i] <= comTag[i];
                specTag_dup[i] <= comTag_dup[i];
            end
        end
        else begin
            for (integer i = 0; i < NUM_ISSUE; i=i+1) begin
                if (IN_issueValid[i] && IN_issueIDs[i] != 0) begin
                    if(IN_issueIDs_info[i].duplicated) begin
                        if(IN_issueIDs_info[i].original) begin
                            specTag[IN_issueIDs[i]] <= IN_issueTags[i];
                        end else begin
                            specTag_dup[IN_issueIDs[i]] <= IN_issueTags[i];
                        end
                    end else begin
                        specTag[IN_issueIDs[i]] <= IN_issueTags[i];
                        specTag_dup[IN_issueIDs[i]] <= IN_issueTags[i];
        ...
        for (integer i = 0; i < NUM_COMMIT; i=i+1) begin
            if (IN_commitValid[i] && IN_commitIDs[i] != 0) begin
                if (IN_mispredFlush) begin
                    if (!IN_mispred) begin
                        if(IN_commitIDs_info[i].duplicated) begin
                            if(IN_commitIDs_info[i].original) begin
                                specTag[IN_commitIDs[i]] <= IN_commitTags[i];
                            end else begin
                                specTag_dup[IN_commitIDs[i]] <= IN_commitTags[i];
                            end
                        end else begin
                                specTag[IN_commitIDs[i]] <= IN_commitTags[i];
                                specTag_dup[IN_commitIDs[i]] <= IN_commitTags[i];
                    ...
                    if(IN_commitIDs_info[i].duplicated) begin
                        if(IN_commitIDs_info[i].original) begin
                            ...
                            comTag_dup[IN_commitIDs[i]] <= IN_commitTags[i];
                        end
                    end else begin
                            comTag[IN_commitIDs[i]] <= IN_commitTags[i];
                            comTag_dup[IN_commitIDs[i]] <= IN_commitTags[i];
